...
void upsdrv_updateinfo(void)
{
	...
	dstate_setinfo("input.voltage", "%.1f", status.utility_voltage);
	dstate_setinfo("output.voltage", "%.1f", status.output_voltage);
	dstate_setinfo("ups.load", "%d", status.percent_load);
	dstate_setinfo("battery.charge", "%d", status.battery_capacity);
	dstate_setinfo("battery.voltage", "%.1f", status.battery_voltage);
	dstate_setinfo("ups.temperature", "%d", status.cabinet_temperature);
	dstate_setinfo("input.frequency", "%.1f", status.utility_frequency);
	dstate_setinfo("output.frequency", "%.1f", status.output_frequency);
